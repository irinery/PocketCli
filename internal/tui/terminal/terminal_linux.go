...
// Termios is the Linux terminal state used by the TUI terminal package.
...
func writeTermios(fd int, state Termios) error {
	...
	for {
		_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), uintptr(syscall.TCSETS), uintptr(unsafe.Pointer(&raw)))
		if errno == syscall.EINTR {
			continue
		}
		if errno != 0 {
