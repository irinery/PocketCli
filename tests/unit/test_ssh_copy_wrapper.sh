...
OUT1=$(env HOME="$HOME_DIR" PATH="$MOCKBIN:/usr/bin:/bin" sh "$HOME_DIR/.pocketcli/scripts/ssh/copy.sh" node1:/tmp/a ./b)
printf '%s' "$OUT1" | grep -F 'OPEN:--run copy node1 node1:/tmp/a ./b' >/dev/null

OUT2=$(env HOME="$HOME_DIR" PATH="$MOCKBIN:/usr/bin:/bin" sh "$HOME_DIR/.pocketcli/scripts/ssh/copy.sh" ./a ./b)
printf '%s' "$OUT2" | grep -F 'SCP:-r ./a ./b' >/dev/null

echo "PASS ssh copy wrapper routes remote copy through policy and preserves local fallback"
