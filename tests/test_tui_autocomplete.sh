...
run_helper() {
    COMMAND="$1"
    sh -c '. "$1"; POCKETCLI_DIR="$2"; TUI_INPUT_HISTORY_LIMIT=50; eval "$3"' sh "$MENU_DEFS" "$POCKET_HOME" "$COMMAND"
}
...
MODE=$(stat -f '%Lp' "$POCKET_HOME/state/tui-input-history" 2>/dev/null || stat -c '%a' "$POCKET_HOME/state/tui-input-history")
[ "$MODE" = "600" ]
