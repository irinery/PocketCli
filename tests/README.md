...
1. **Novo módulo interno (`internal/<modulo>`)**
   ...
   - Para rodar isoladamente a etapa 02 do orquestrador `pocket connect`, use `env GOCACHE=/tmp/pocketcli-go-build-cache go test ./internal/connect ./cmd/pocket`.
   ...
   - Pré-requisitos da etapa 02 do `pocket connect`: toolchain Go suportado pelo projeto; os testes unitários usam doubles para `tmux`, `ssh` e `tailscale`, então não exigem esses binários reais. Variáveis de ambiente: `GOCACHE` em ambiente sandboxado.
